...
export const OrdersTable = ({ orders = [], onCancelOrder, cancellingId }) => {
  if (orders.length === 0) {
    return (
      <div className="py-12 text-center text-slate-400 text-xs bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
        ...
        <span>No orders currently recorded for your account.</span>
      </div>
    );
  }
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'open':
        return <Badge variant="blue" dot>Open Active</Badge>;
      case 'filled':
        return <Badge variant="emerald" dot>Fully Cleared</Badge>;
      case 'partially_filled':
        return <Badge variant="amber" dot>Partial Fill</Badge>;
      case 'cancelled':
            ...
            return (
              <tr key={order.id} className="hover:bg-slate-50/60 transition-colors">
                <td className="py-3 px-4">
                  ...
                  >
                    {order.side === 'offer' ? '☀️ Offer (Sell)' : '⚡️ Bid (Buy)'}
                  </span>
                </td>
